...
{         	
    ...
    if(num < 0)
	{
        OLED_ShowChar(x,y,'0'- 3,size2,1);
		num =fabs(num);
	}
    else
	OLED_ShowChar(x,y,' ',size2,1);//第一位显示符号

	temp1 = (int)temp;
	temp2 = num - temp1;
	len1 = len - 6;//len1为整数部分位数，若显示数位需要扩展，修改该行

	OLED_ShowChar(x + size2/2*4,y,'0'- 2,size2,1);//浮点数的第5位显示小数点
	...
	OLED_ShowChar(x+(size2/2)*4,y,((int)(temp2*10)%10) + '0',size2,1); //小数第一位
	OLED_ShowChar(x+(size2/2)*5,y,((int)(temp2*100)%10) + '0',size2,1); //小数第2位
	OLED_ShowChar(x+(size2/2)*6,y,((int)(temp2*1000)%10) + '0',size2,1); //小数第3位
	OLED_ShowChar(x+(size2/2)*7,y,((int)(temp2*10000)%10) + '0',size2,1); //小数第4位
} 
